...
mvn package -pl template-fastgen-scaffold -DskipTests
```
...
```bash
# 在当前目录下生成 my-app/ 项目
java -jar template-fastgen-scaffold/target/template-fastgen-scaffold-2.4.2.1-SNAPSHOT.jar \
  --artifactId=my-app \
  --groupId=com.example \
  --appPackage=com.example.myapp

# 指定输出目录时，项目生成在 该目录/artifactId/
java -jar template-fastgen-scaffold-*.jar --outputDir=./projects --artifactId=my-app
```

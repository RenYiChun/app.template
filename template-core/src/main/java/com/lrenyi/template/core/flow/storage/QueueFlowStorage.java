 ...
 * {@link com.lrenyi.template.core.flow.manager.FlowCacheManager} 中的
 * {@code storageEgressExecutor}（单物理线程）完成：
 * 在构造函数中按配置的 TTL 周期向该 executor 提交排空任务，从队列取数 →
 * {@link FlowFinalizer#submitBodyOnly}（acquire 由 FlowGlobalExecutor 处理），
 * 与 Caffeine 驱逐路径共用同一离场线程，离场语义一致。
 * <p>
 * 适用于：顺序消费、削峰填谷。不支持按 Key 检索，{@link #remove(String)} 使用接口默认实现。
 * <p>
 * 若由框架通过
 * {@link com.lrenyi.template.core.flow.manager.FlowCacheManager#getOrCreateStorage}
 * 创建并传入 finalizer、jobId
 * 、storageEgressExecutor、drainIntervalMs、drainScheduler，
 * 则在构造函数中向 FlowManager 的 queueDrainScheduler 注册定时任务（按 drainIntervalMs 周期向
 * storageEgressExecutor 提交 drainLoop），与
 * Caffeine 共用离场线程；
 */
@Slf4j
public class QueueFlowStorage<T> implements FlowStorage<T> {
    ...
    /**
     * 框架创建时使用：传入
     * finalizer、jobId、storageEgressExecutor、drainIntervalMs、drainScheduler 时，
     * 使用定时调度（scheduleAtFixedRate）按 drainIntervalMs 周期向 storageEgressExecutor 提交
     * drainLoop，与 Caffeine 共用离场线程。
     *
     ...
     * @param drainIntervalMs 排空周期（毫秒），与 Flow 配置中的 ttlMill 对齐；<=0 则不注册
     */
    ...
    /**
     * 在 storageEgressExecutor 单线程上执行：非阻塞取队直到空，每条 submitBodyOnly（acquire 由
     * FlowGlobalExecutor 处理）。
     */

...
public record FlowProgressSnapshot(String jobId, long totalExpected, // 1. 预期总量
    ...
    /**
     * 业务成功率 (Success Rate / Health)。
     * 已移除 activeEgress/passiveEgress 计数，此方法保留兼容性，始终返回 1.0。
     *
     * @deprecated 框架不再统计主动/被动出口，请通过 Micrometer 指标 {@code egress.active} / {@code egress.passive} 观测
     */
    @Deprecated
    ...
    /**
     * 按消费原因获取被动出口数量。
     * 已移除 passiveEgressByReason 计数，此方法保留兼容性，始终返回 0。
     *
     * @param reason 消费原因 EgressReason 枚举名（如 TIMEOUT、EVICTION）
     * @return 0（框架不再按原因统计，请通过 Micrometer 指标 {@code egress.passive} 的 reason 标签观测）
     * @deprecated 框架不再按原因统计，请使用 Micrometer 指标
     */
    @Deprecated

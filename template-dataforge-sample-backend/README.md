...
2. 启动 `template-dataforge-sample-ui`：`cd ..\template-dataforge-sample-ui && npm install && npm run dev`（端口
   3000，proxy 到 8080）
3. 或业务前端：配置 `VITE_API_BASE_URL=http://localhost:8080`，
   `createDataforge({ client: { baseURL: 'http://localhost:8080', apiPrefix: '/api' } })`

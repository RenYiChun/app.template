...
**模块职责**：`template-dataforge` 核心无 JPA/Mongo 依赖；`template-dataforge-jpa`、`template-dataforge-mongodb` 均为可选扩展，对称结构。
...
- 示例：`ProductsCrudService extends DelegatingEntityCrudService implements PathSegmentAwareCrudService`，`getPathSegment()="products"`，只重写 `create()` → `products` 走自定义逻辑，其余实体仍走 MongoEntityCrudService
...
- `target()` 逻辑：1）`pathSegment` 在 pathSegmentToDelegate 中 → 返回；2）`storageType` 在 storageTypeToDelegate 中 → 返回；3）否则 defaultService
...
**现状**：RBAC 实体（Permission、Role、RolePermission、UserRole、OperationLog）使用 JPA 注解并继承 BaseEntity，当前 model 依赖 JPA。
...
**分析**：`DataforgePersistable` 已约定语义字段：`createTime`、`updateTime`、`createBy`、`updateBy`。JPA 用 `@CreatedBy`、`@LastModifiedBy` 等；Mongo 可用 `@CreatedDate` 或手动填充。接口只约束字段名与语义，具体注解由各基类实现。
...
**结论**：**存在，建议作为后续增强**。首版使用 skip/limit；后续可在 `ListCriteria` 或 Mongo 实现中支持游标参数（如 `cursorId`、`lastId`）。

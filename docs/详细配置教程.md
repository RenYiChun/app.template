...
- **template-web**: Web 应用模块
- **template-service**: 微服务模块
...
app:
  template:
    config:
      enable: true  # 默认为 true，设置为 false 可禁用整个框架
```
...
public class CustomSecurityConfig {
    ...
    @Bean
    public RolePermissionService customRolePermissionService() {
        return new CustomRolePermissionService();
    }
}
```

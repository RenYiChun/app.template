...
import com.lrenyi.template.entityplatform.meta.FieldMeta;
...
/**
 * 将实体列表按 {@link EntityMeta} 中可导出字段（未标 {@link com.lrenyi.template.entityplatform.annotation.ExportExclude}）导出为 Excel（.xlsx）。
...
public final class ExcelExportSupport {
    ...
    public static byte[] toExcel(EntityMeta meta, List<?> data, ObjectMapper objectMapper) throws Exception {
        List<FieldMeta> exportFields = meta.getFields().stream()
        ...
        List<String> columnNames = exportFields.stream().map(FieldMeta::getName).collect(Collectors.toList());
    ...
    private static final class PoiReflect {
        ...
        PoiReflect(Class<?> workbookClass, Class<?> sheetClass, Class<?> rowClass, Class<?> cellClass)
                throws Exception {
        ...
        void setCellValue(Object row, int colIndex, Object value) throws Exception {
            ...
            if (value instanceof Number n) {
                setCellValueNum.invoke(cell, n.doubleValue());
            } else if (value instanceof Boolean b) {
                setCellValueBool.invoke(cell, b);
            } else {
                setCellValueStr.invoke(cell, value.toString());
            }
        }
